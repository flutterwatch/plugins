...
// Firebase Apple SDK's `FirebaseAuth` product (source-built; the SDK manifest
// supports `.watchOS(.v7)`). The Objective-C FFI layer imports `FirebaseAuth`
// via its module and links it into the app.
import PackageDescription

let package = Package(
    name: "firebase_auth_watchos",
    platforms: [
        .watchOS("7.0"),
    ],
    products: [
        .library(name: "firebase-auth-watchos", targets: ["firebase_auth_watchos"]),
    ],
    dependencies: [
        .package(
    ...
    targets: [
        .target(
            ...
            linkerSettings: [
                ...
                // the harvested FirebaseAuth/FirebaseCore/GoogleUtilities
